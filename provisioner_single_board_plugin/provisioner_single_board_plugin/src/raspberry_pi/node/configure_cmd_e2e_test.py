...
class RaspberryPiNodeCliE2ETestShould(unittest.TestCase):
    ...
    def test_e2e_install_anchor_on_remote_successfully(self):
        ...
        self.assertIn("Mock raspi-config: do_boot_splash 1", output)
        self.assertIn("Mock raspi-config: do_overscan 1", output)
        self.assertIn("Mock raspi-config: do_spi 1", output)
        self.assertIn("Mock raspi-config: do_i2c 1", output)
        self.assertIn("Mock raspi-config: do_onewire 1", output)
        self.assertIn("Mock raspi-config: do_audio 0", output)
        self.assertIn("Mock raspi-config: do_configure_keyboard us", output)
        self.assertIn("Mock raspi-config: do_change_timezone UTC", output)
