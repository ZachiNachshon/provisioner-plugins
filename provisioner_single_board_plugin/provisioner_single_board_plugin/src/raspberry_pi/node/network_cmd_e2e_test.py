...
class RaspberryPiNodeNetworkE2ETestShould(unittest.TestCase):
    ...
    def test_e2e_configure_network_settings_without_wifi(self):
        """Test network configuration with only hostname setting."""
        ...
        try:
            ...
            # Verify only hostname was configured
            self.assertIn("Mock raspi-config: do_hostname test-node", output)
            self.assertNotIn("Mock raspi-config: do_wifi_country", output)
            self.assertNotIn("Mock raspi-config: do_wifi_ssid_passphrase", output)

        except Exception as e:
            print(f"Test failed after {time.time() - start_time} seconds: {str(e)}")
            raise
