...
class FakePaths(Paths):
    ...
    def _create_fake(ctx: Context) -> "FakePaths":
        ...
        paths.get_path_from_exec_module_root_fn = lambda relative_path=None: paths.append_relative_path(paths.path_exec_module_root, relative_path) if relative_path else paths.path_exec_module_root
        paths.get_path_abs_to_module_root_fn = lambda package_name, relative_path=None: paths.append_relative_path(paths.path_abs_module_root, relative_path) if relative_path else paths.path_abs_module_root
        paths.get_path_relative_from_module_root_fn = lambda package_name, relative_path=None: paths.append_relative_path(paths.path_relative_module_root, relative_path) if relative_path else paths.path_relative_module_root
