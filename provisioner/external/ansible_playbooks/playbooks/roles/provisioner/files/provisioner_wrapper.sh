...
uninstall_via_pip() {
  local pkg_name=$1
  local pkg_version=$2
  log_info "Uninstalling pip package. name: ${pkg_name}"
...
install_via_github_release() {
  ...
  if is_directory_exist "${pkg_folder_path}"; then
    log_info "Removing local pip pkg. path: ${pkg_folder_path}"
    cmd_run "rm -rf ${pkg_folder_path}"
  fi

  log_info "Downloading a GitHub release. dest: ${pkg_folder_path}"
  github_download_release_asset \
    "${ENV_GITHUB_OWNER}" \
  ...
  if is_dry_run || is_file_exist "${pkg_folder_path}/${asset_name}"; then
    ...
    log_info "Installing from GitHub release. name: ${asset_name}, version: ${pkg_version}"
...
install_via_pip() {
  ...
  log_info "Installing from pip registry. name: ${pkg_name}, version: ${pkg_version}"
...
is_pip_installed() {
  local pkg_name=$1
  log_info "Checking if installed from pip. name: ${pkg_name}"
...
install_or_update() {
  ...
  if ! is_pip_installed "${pkg_name}"; then
    log_info "Pip package is not installed. name: ${pkg_name}"
    install_package "${pkg_name}" "${pkg_version}"
  else
    log_info "Trying to read pip package version. name: ${pkg_name}"
    local current_version=$(pip_get_package_version "${pkg_name}")
    if [[ "${current_version}" == "${ENV_PROVISIONER_VERSION}" ]]; then
      log_info "Found installed pip package with expected version. name: ${pkg_name}, version: ${pkg_version}"
    else
      log_info "Pip package does not have the expected version. name: ${pkg_name}, current_version: ${current_version}, expected: ${pkg_version}"
      install_package "${pkg_name}" "${pkg_version}"
    fi
  fi
}
...
install_provisioner_engine() {
  ...
  if is_tool_exist "${ENV_PROVISIONER_BINARY}"; then
    log_info "Found installed binary. name: ${ENV_PROVISIONER_BINARY}, path: $(which "${ENV_PROVISIONER_BINARY}")"
  else
...
install_provisioner_plugins() {
  ...
  log_info "Installing required plugins: ${ENV_REQUIRED_PLUGINS}"
...
main() {
  ...
  log_info "Running ${ENV_PROVISIONER_BINARY} command..."
