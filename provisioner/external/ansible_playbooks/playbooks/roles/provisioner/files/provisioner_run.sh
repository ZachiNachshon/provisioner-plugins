...
# Title         Provisioner command runner
...
is_run_command() {
  [[ "${ENV_PROVISIONER_ANSIBLE_COMMAND}" == "Run" ]]
}

is_uninstall_command() {
  [[ "${ENV_PROVISIONER_ANSIBLE_COMMAND}" == "Uninstall" ]]
}

should_install_or_update() {
  ! is_tool_exist "provisioner" || [[ ($(provisioner --version) != "${ENV_PROVISIONER_VERSION}") ]]
}

generate_provisioner_github_url() {
  echo "https://${ENV_GIT_ACCESS_TOKEN}:@github.com/${ENV_PROVISIONER_GITHUB_ORGANIZATION}/${ENV_PROVISIONER_GITHUB_REPOSITORY}.git"
}

install_or_update_provisioner() {
  local os_type=$(read_os_type)
  local arch=$(read_arch "x86_64:amd64" "armv:armv6")
  local provisioner_os_arch="${os_type}_${arch}"

  log_info "Installing provisioner CLI. version: ${ENV_PROVISIONER_VERSION}, os_arch: ${provisioner_os_arch}"

  if is_directory_exist "${ENV_LOCAL_PIP_PKG_FOLDER_PATH}"; then
    log_info "Clearing previous installed Python distribution"
    rm -rf "${ENV_LOCAL_PIP_PKG_FOLDER_PATH}"
  fi

  mkdir -p "${ENV_LOCAL_PIP_PKG_FOLDER_PATH}"

  # Create a temporary folder
  local repo_temp_path=$(cmd_run "mktemp -d ${TMPDIR:-/tmp}/provisioner-repo.XXXXXX")
  local cwd=$(pwd)
  cmd_run "cd '${repo_temp_path}' || exit"

  # Download & extract
  log_info "Downloading provisioner to temp directory..."
  local file_name="provisioner_${ENV_PROVISIONER_VERSION}_${provisioner_os_arch}.tar.gz"
  cmd_run "curl -sSL https://github.com/ZachiNachshon/anchor/releases/download/v${ENV_PROVISIONER_VERSION}/${file_name}"
  ...
  log_info "Moving pip package to ${ENV_LOCAL_PIP_PKG_FOLDER_PATH}"
  cmd_run "mv ${file_name} ${ENV_LOCAL_PIP_PKG_FOLDER_PATH}"

  cmd_run "cd '${ENV_LOCAL_PIP_PKG_FOLDER_PATH}' || exit"
  log_info "Unpacking archive..."
  tar -xf "${file_name}"
  mv "${file_name}" provisioner
...
run_provisioner() {
  ...
  cmd_run "${ENV_LOCAL_BIN_FOLDER_PATH}/anchor ${ENV_PROVISIONER_RUN_ARGS}"
  # Try the next command if the previous fails printing to stdout on Ansible run
  # cmd_run "bash -ilc anchor ${ENV_PROVISIONER_RUN_ARGS} 2>&1"
}
...
main() {
  ...
  ls -lah
  cat "${ENV_PROVISIONER_BIN_FILE_NAME}"
