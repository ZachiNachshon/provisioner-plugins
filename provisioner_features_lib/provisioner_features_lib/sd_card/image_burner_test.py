...
SD_CARD_IMAGE_BURNER_RUNNER_PATH = (
...
class ImageBurnerTestShould(unittest.TestCase):
    ...
    def test_main_flow_run_actions_have_expected_order(
        ...
        ImageBurnerCmdRunner().run(
            env.get_context(), self.create_fake_image_burner_args(), env.get_collaborators()
        )
    ...
    def test_ask_user_to_select_block_devices(self, 
        prompt_for_block_device_name_call: mock.MagicMock,
        verify_block_device_name_call: mock.MagicMock) -> None:
        env = TestEnv.create()
        ImageBurnerCmdRunner()._ask_user_to_select_block_devices(env.get_context(), env.get_collaborators(), BLOCK_DEVICES_OUTPUT)
    ...
    def test_download_image(self) -> None:
        env = TestEnv.create()
        env.get_collaborators().http_client().mock_download_file_response(ARG_IMAGE_DOWNLOAD_URL, ARG_IMAGE_DOWNLOAD_PATH)
        ImageBurnerCmdRunner()._download_image(env.get_context(), ARG_IMAGE_DOWNLOAD_URL, ARG_IMAGE_DOWNLOAD_PATH, env.get_collaborators())
    ...
    def test_burn_image_by_os_linux(self, 
        run_pre_burn_approval_flow_call: mock.MagicMock,
        burn_image_linux_call: mock.MagicMock) -> None:
        env = TestEnv.create(ctx=Context.create(os_arch=OsArch(os=LINUX)))
        ImageBurnerCmdRunner()._burn_image_by_os(env.get_context(), SELECTED_BLOCK_DEVICE, ARG_IMAGE_DOWNLOAD_PATH, env.get_collaborators())
    ...
    @mock.patch(f"{SD_CARD_IMAGE_BURNER_RUNNER_PATH}._run_pre_burn_approval_flow")
    def test_burn_image_by_os_darwin(self, 
        run_pre_burn_approval_flow_call: mock.MagicMock,
        burn_image_darwin_call: mock.MagicMock) -> None:
    ...
    def test_run_pre_burn_approval_flow(self, ask_to_verify_block_device_call: mock.MagicMock) -> None:
        env = TestEnv.create()
        ImageBurnerCmdRunner()._run_pre_burn_approval_flow(env.get_context(), SELECTED_BLOCK_DEVICE, env.get_collaborators())
    ...
    def test_burn_image_linux(self) -> None:
        env = TestEnv.create()
        ImageBurnerCmdRunner()._burn_image_linux(SELECTED_BLOCK_DEVICE, ARG_IMAGE_DOWNLOAD_PATH, env.get_collaborators())
    ...
    def test_burn_image_darwin(self) -> None:
        env = TestEnv.create()
        ImageBurnerCmdRunner()._burn_image_darwin(SELECTED_BLOCK_DEVICE, ARG_IMAGE_DOWNLOAD_PATH, env.get_collaborators())
        env.get_collaborators().printer().assert_outputs(
            [
    ...
    def test_ask_to_verify_block_device(self) -> None:
        ...
        env.get_collaborators().prompter().assert_yes_no_prompt(f"ARE YOU SURE YOU WANT TO FORMAT BLOCK DEVICE '{SELECTED_BLOCK_DEVICE}'")
