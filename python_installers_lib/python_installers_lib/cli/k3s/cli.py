...
from python_installers_lib.installer.cmd.installer_cmd import UtilityInstallerCmd, UtilityInstallerCmdArgs

def k3s_server(
    get_node_token: str = typer.Argument(None, 
        show_default=False, help="Read k3s server node token (NODE_TOKEN)"),
) -> None:
    """
    ...
    Evaluator.eval_installer_cli_entrypoint_step(
        ...
        call=lambda: UtilityInstallerCmd().run(
            ctx=CliContextManager.create(), 
            args=UtilityInstallerCmdArgs(utilities=["k3s-server"])),
    )
...
def k3s_agent(
    ...
    Evaluator.eval_installer_cli_entrypoint_step(
        ...
        call=lambda: UtilityInstallerCmd().run(
            ctx=CliContextManager.create(), 
            args=UtilityInstallerCmdArgs(utilities=["k3s-agent"])),
    )