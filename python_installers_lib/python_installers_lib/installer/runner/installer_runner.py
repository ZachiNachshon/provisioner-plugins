...
from python_installers_lib.installer.installables import (
    Installables,
...
class UtilityInstallerCmdRunner:
    def run(
        ...
        collaborators.__summary.add_values("utilities", utilities_to_install)
        ...
        selected_run_env = Evaluator.eval_step_return_failure_throws(
            call=lambda: self._resolve_run_environment(args.remote_opts.environment, collaborators.__prompter),
            ctx=ctx,
            err_msg="Could not resolve run environment",
        )
        collaborators.__summary.add_value("run_env", selected_run_env)
    ...
    def _run_local_installation(
        ...
        for utility in utilities:
            ...
            self._print_pre_install_summary(
                utility.name, ctx.is_auto_prompt(), collaborators.__printer, collaborators.__prompter, collaborators.__summary
            )
            self._install_utility_locally(
                ctx=ctx,
                utility=utility,
                github=collaborators.__github,
                process=collaborators.__process,
                printer=collaborators.__printer,
            )
        ...
        for utility in utilities:
            ...
            output = collaborators.__printer.progress_indicator.status.long_running_process_fn(
                call=lambda: collaborators.__ansible_runner.run_fn(
                    ...
                    playbook_path=collaborators.__io.get_path_relative_from_module_root_fn(
                        __name__, ProvisionerRunAnsiblePlaybookRelativePath
                    ),
                    extra_modules_paths=[collaborators.__io.get_path_abs_to_module_root_fn(__name__)],
