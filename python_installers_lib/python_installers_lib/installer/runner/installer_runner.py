...
from python_installers_lib.installer.installables import Installables, SupportedInstallables
...
class UtilityInstallerCmdRunner:
    ...
    def _install_utility_locally(
        self, 
        ctx: Context,
        utility: Installables.InstallableUtility, 
        github: GitHub,
        process: Process, 
        printer: Printer):
        ...
        if not latest_version:
            raise FailedToResolveLatestVersionFromGitHub(f"Failed to resolve latest version from GitHub. owner: {utility.owner}, repo: {utility.repo}")
